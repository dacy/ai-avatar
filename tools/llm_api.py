...
def create_llm_client(provider="openai"):
    if provider == "openai":
        ...
        # Ollama doesn't need a client object, we'll use requests directly
...
def query_ollama(prompt: str, model: str, base_url: str) -> Optional[str]:
    ...
    try:
        ...
        print(f"Sending request to Ollama API: {url}", file=sys.stderr)
    ...
    try:
        ...
        if model is None:
            if provider == "openai":
                ...
                model = os.getenv('OLLAMA_MODEL', 'llama3')  # Default to llama3 if not specified
        
        # Handle Ollama separately
        if provider == "ollama":
            return query_ollama(prompt, model, client)
...
def main():
    ...
    parser.add_argument('--provider', choices=['openai','anthropic','gemini','local','deepseek','azure','siliconflow','ollama'], default='openai', help='The API provider to use')
    ...
    if not args.model:
        if args.provider == 'openai':
            ...
            args.model = os.getenv('OLLAMA_MODEL', 'llama3')  # Default to llama3 if not specified
